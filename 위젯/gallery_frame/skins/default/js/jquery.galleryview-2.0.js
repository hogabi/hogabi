...
function dj(input){
	if(window.console && window.console.debug) { window.console.debug(input); }
}
...
/************************************************/	
	...
	//Construct gallery panels from '.panel' <div>s
		function buildPanels() {
			...
			j_panels.each(function(i){
				...
				//xe customize zirho
				j_frames.eq(i).find('img').eq(0).attr('alt') ? $(this).css('cursor','pointer').click(function(){location.href=j_frames.eq(i).find('img').eq(0).attr('alt');}) : '';
	...
	//Construct filmstrip from '.filmstrip' <ul>
		function buildFilmstrip() {
			...
			var pointer = $('<div class="iePngFix"></div>');
...
/************************************************/
		return this.each(function() {
			...
			// xe customizing zirho
			theme_path = opts.theme_path + '/themes/';	
			...
			if(opts.show_panels) {
				for(i=j_frames.length-1;i>=0;i--) {
					if(j_frames.eq(i).find('.panel-content').length>0) {
						...
						//xe customize
						im = $('<img />'); //im = $('<img />').addClass('iePngFix');
						j_frames.eq(i).find('img').eq(0).attr('alt')? im.css('cursor','pointer').attr('onclick','location.href=\'' + j_frames.eq(i).find('img').eq(0).attr('alt') + '\';'):'';
	...
	$.fn.galleryView.defaults = {
		...
		theme_path: '/widgets/gallery_frame/skins/default/js'
	};
})(jQuery);
