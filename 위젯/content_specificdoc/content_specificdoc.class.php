...
//            if(count($attrs)) $title = sprintf("<span style=\"%s\">%s</span>", implode(';', $attrs), htmlspecialchars($title));
        ...
        function getNewCommentInfo() {
            return $this->get('new_comment_count');
        }
		function setNewCommentInfo ($document_srl) {
//			print "setNewCommentInfo 시작 : " . $document_srl ;
//			print "<b>";
			...
			$query  = " SELECT count(*) as comment_count from xe_comments "; 
			$query .= " where document_srl = " . $document_srl;
			$query .= " and regdate >= date_add(now(), interval -48 hour) ";

			$result = $oDB->_query($query);
			$output_comment = $oDB->_fetch($result);
						
			$new_comment_count = $output_comment->comment_count;
			if ($new_comment_count <= 0) $new_comment_count = 0;



//			print "count ". $output_comment->comment_count;
//			print_r ($output_comment);

			$this->add('new_comment_count',$new_comment_count);
		}
