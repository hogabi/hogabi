...
class boardView extends board
{
	...
	function dispBoardCategoryList(){
		...
		if($this->module_info->use_category=='Y')
		{
           ...
           // 2018.01.22 전체게시판 기능 추가 : 조호갑
            if ($this->mid == 'board_total_news'){
                $args->module_srl = implode(',', array(108, 220, 251, 253, 280, 1371, 2525, 4722, 4801));
                
                Context::set('category_list', $oDocumentModel->getCategoryList(108) + 
                                              $oDocumentModel->getCategoryList(288) + 
                                              $oDocumentModel->getCategoryList(220) + 
                                              $oDocumentModel->getCategoryList(251) + 
                                              $oDocumentModel->getCategoryList(253) + 
                                              $oDocumentModel->getCategoryList(1371) + 
                                              $oDocumentModel->getCategoryList(2525) );
            } else {
